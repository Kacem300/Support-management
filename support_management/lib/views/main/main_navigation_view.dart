...
class _MainNavigationViewState extends State<MainNavigationView> {
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                BottomNavigationBar(
                  currentIndex:
                      _getBottomNavIndex() == -1 ? 0 : _getBottomNavIndex(),
                  onTap: (index) {
                    // Map bottom nav index to actual page index
                    int actualIndex;
                    switch (index) {
                      case 0:
                        actualIndex = 0; // Accueil
                        break;
                      case 1:
                        actualIndex = 1; // Tickets
                        break;
                      case 2:
                        actualIndex = 2; // Empty space - should not happen
                        return; // Do nothing for empty space
                      case 3:
                        actualIndex = 3; // Clients
                        break;
                      case 4:
                        actualIndex = 4; // Menu
                        break;
                ...
                if (_currentIndex != 2) // Only show for non-Messages tabs
                  Container(
                    height: 3,
                    width: 50, // Match main_page.dart width
                    margin: EdgeInsets.only(
                      left: _getIndicatorPosition(),
                      right: MediaQuery.of(context).size.width -
                          _getIndicatorPosition() -
                          50, // Match main_page.dart logic
                      top: 0,
                    ),
                    decoration: BoxDecoration(
                      color: Colors.black,
                      borderRadius:
                          BorderRadius.circular(1.5), // Match main_page.dart
                    ),
                  ),
              ],
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Messages',
                    style: TextStyle(
                      fontSize: 11, // Match main_page.dart
                      color: _currentIndex == 2
                          ? const Color(0xFF4ECDC4) // Selected color
                  ...
                  if (_currentIndex == 2)
                    Container(
                      margin:
                          const EdgeInsets.only(top: 1), // Match main_page.dart
                      height: 2, // Match main_page.dart height
                      width: 50, // Match main_page.dart width
                      decoration: BoxDecoration(
                        color: Colors.black,
                        borderRadius:
                            BorderRadius.circular(1), // Match main_page.dart
                      ),
                    ),
                ],
              ),
  ...
  int _getBottomNavIndex() {
    ...
    switch (_currentIndex) {
      ...
      case 3:
        return 2; // Clients (position 3 in bottom nav)
      case 4:
        return 3; // Menu (position 4 in bottom nav)
      default:
        return 0;
    }
  }
